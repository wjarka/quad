...
class FfmpegX264Recorder(FfmpegRecorder):
    ...
    def prepare_ffmpeg_command(self, path, source_name):
        ...
        return prepare_command(
            "ffmpeg -hide_banner -loglevel error 		 -use_wallclock_as_timestamps 1"
            " -rtbufsize 1024M -thread_queue_size 8192 -f libndi_newtek -i"
            f" '{source_name}' 		 -codec:v libx264 -threads 0 -codec:a aac -g 250"
            f" -keyint_min 250 		 -preset medium -b:v {self.bitrate} -bufsize 1M"
            f" -maxrate {self.bitrate} '{path}' -y"
        )
...
class FfmpegVaapiRecorder(FfmpegRecorder):
    def can_record(self):
        ...
        output, status = pexpect.run(
            "ffmpeg -hide_banner -loglevel error -init_hw_device"
...
class FfmpegHevcVaapiRecorder(FfmpegVaapiRecorder):
    ...
    def prepare_ffmpeg_command(self, path, source_name):
        ...
        return prepare_command(
            "ffmpeg -hide_banner -loglevel error -init_hw_device"
            " vaapi=foo:/dev/dri/renderD128             -use_wallclock_as_timestamps 1"
            " -rtbufsize 1024M -thread_queue_size 8192 -f libndi_newtek -i"
            f" '{source_name}'             -codec:v hevc_vaapi -threads 0 -codec:a aac"
            " -g 250 -keyint_min 250 -profile:v main             -vf"
            f" 'format=nv12,hwupload' -b:v {self.bitrate} -bufsize 1M -maxrate"
            f" {self.bitrate} '{path}' -y"
        )
...
class FfmpegH264VaapiRecorder(FfmpegVaapiRecorder):
    ...
    def prepare_ffmpeg_command(self, path, source_name):
        ...
        return prepare_command(
            ...
            " vaapi=foo:/dev/dri/renderD128             -use_wallclock_as_timestamps 1"
            " -rtbufsize 1024M -thread_queue_size 8192 -f libndi_newtek -i"
            f" '{source_name}'             -codec:v h264_vaapi -threads 0 -codec:a aac"
            " -g 250 -keyint_min 250 -profile:v main             -vf"
            f" 'format=nv12,hwupload' -b:v {self.bitrate} -bufsize 1M -maxrate"
            f" {self.bitrate} '{path}' -y"
        )
...
class FfmpegH264QsvRecorder(FfmpegRecorder):
    ...
    def prepare_ffmpeg_command(self, path, source_name):
        ...
        return prepare_command(
            "ffmpeg -hide_banner -loglevel error -hwaccel qsv -hwaccel_output_format"
            " qsv             -use_wallclock_as_timestamps 1 -rtbufsize 1024M"
            f" -thread_queue_size 8192 -f libndi_newtek -i '{source_name}'            "
            " -codec:v h264_qsv -threads 0 -codec:a aac -g 250 -keyint_min 250"
            f" -profile:v main             -preset medium -b:v {self.bitrate} -bufsize"
            f" 1M -maxrate {self.bitrate} '{path}' -y"
        )
...
class ObsRecorder(Recorder):
    ...
    def can_record(self):
        ...
        return (
            ...
            or not current_app.config["OBS_SKIP_RECORDING_WHEN_STREAMING"]
        )
    ...
    def _connect(self):
        ...
        try:
            self.obs = obs.ReqClient(
                host=current_app.config["OBS_WEBSOCKET_IP"],
    ...
    def start(self, game):
        ...
        if self.connect():
            if (
                not self._is_streaming()
                or not current_app.config["OBS_SKIP_SWITCHING_SCENES_WHEN_STREAMING"]
            ):
                self.obs.set_current_program_scene(
                    current_app.config["OBS_RECORDING_SCENE"]
                )
    ...
    def stop(self):
        ...
        source_path = os.path.join(
            current_app.config["OBS_RECORDING_BASE_DIR"],
        ...
        output, status = pexpect.run(
            f'mv "{source_path}" "{destination_path}"', withexitstatus=True
        )
        if status == 0:
