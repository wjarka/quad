...
class IsAlive(MatcherAbstract):
    ...
    def __init__(self):
        ...
        self.template = cv2.imread(
            os.path.join(self._assets_path, "templates/template-healthbarmask.png"), 0
        )
...
class ChampionMatcher(MatcherAbstract):
    ...
    def _load_champions(self):
        ...
        for champion in champions:
            self._championTemplates[champion.name] = cv2.imread(
                os.path.join(current_app.root_path, champion.template_path), 0
            )

    def _evaluate(self, image, champion):
        result = cv2.matchTemplate(
            image, self._championTemplates[champion], cv2.TM_CCOEFF_NORMED
        )
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
...
class Desktop(MatcherAbstract):
    def match(self, frame):
        template = cv2.imread(
            os.path.join(self._assets_path, "icons/windows-icon.png"), 0
        )
        result = cv2.matchTemplate(
            cv2.cvtColor(frame[1043:1080, 0:37], cv2.COLOR_BGR2GRAY),
            template,
...
class Scoreboard(MatcherAbstract):
    def __init__(self):
        super().__init__()
        self.template = cv2.imread(
            os.path.join(self._assets_path, "templates/duel.png"), 0
        )

    def match(self, frame):
        res = cv2.matchTemplate(
            cv2.cvtColor(frame[40:140, 20:120], cv2.COLOR_BGR2GRAY),
            self.template,
            cv2.TM_CCOEFF_NORMED,
        )
...
class DuelEndScoreboard(Scoreboard):
    ...
    def match(self, frame):
        match, data = super().match(frame)
        if match:
            text = self.ocr.get_text_hsv(
                frame[0:45, 110:300], [0, 0, 163], [179, 255, 255]
            ).replace("\n", "")
...
class MainMenu(MatcherAbstract):
    def __init__(self):
        super().__init__()
        self.template = cv2.imread(
            os.path.join(self._assets_path, "templates/contacts.png"), 0
        )

    def match(self, frame):
        result = cv2.matchTemplate(
            cv2.cvtColor(frame[1030:1060, 130:240], cv2.COLOR_BGR2GRAY),
            self.template,
            cv2.TM_CCOEFF_NORMED,
        )
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
...
class MenuLoading(MatcherAbstract):
    def __init__(self):
        super().__init__()
        self.template = cv2.imread(
            os.path.join(self._assets_path, "templates/menu-loading.png"), 0
        )

    def match(self, frame):
        result = cv2.matchTemplate(
            cv2.cvtColor(frame[480:680, 730:1200], cv2.COLOR_BGR2GRAY),
            self.template[480:680, 730:1200],
            cv2.TM_CCOEFF_NORMED,
        )
        min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
        if max_val > 0.95:
...
class WarmupEnd(MatcherAbstract):
    ...
    def get_player_champion(self, frame):
        ...
        champY = 28
        champX = 679
        return self.champion_matcher.identify_champion(
            frame[champY : champY + champImageSize, champX : champX + champImageSize]
        )
    ...
    def get_corrected_name(self, name):
        ...
        model = db.session.scalar(
            select(OcrVocabulary).where(OcrVocabulary.text_read == name)
        )
        if model:
            return model.text
        return name
    ...
    def match(self, frame):
        ...
        if mask[0, 0] == 255:
            ...
            p_name = self.get_corrected_name(
                self.ocr.get_text_hsv(frame[92:114, 660:875], lower, higher).replace(
                    "\n", ""
                )
            )
            o_name = self.get_corrected_name(
                self.ocr.get_text_hsv(frame[92:114, 1040:1238], lower, higher).replace(
                    "\n", ""
                )
            )
            if not self.stats.player_exists(p_name) or not self.stats.player_exists(
                o_name
            ):
                if "DISCORD_WEBHOOK_STATUS" in current_app.config:
                    ...
                    send_message(
                        current_app.config["DISCORD_WEBHOOK_STATUS"],
                        f"{p_name} or {o_name} is an incorrect player name.",
                        frame,
                    )
...
class MapLoading(MatcherAbstract):
    ...
    def _load_templates(self):
        ...
        self.mapTemplates = {}
        for map in maps:
            self.mapTemplates[map.code] = cv2.imread(
                os.path.join(current_app.root_path, map.template_path), 0
            )

    def match(self, frame):
        for map, template in self.mapTemplates.items():
            result = cv2.matchTemplate(
                cv2.cvtColor(frame[105:155, 170:590], cv2.COLOR_BGR2GRAY),
                template[105:155, 170:590],
                cv2.TM_CCOEFF_NORMED,
            )
            min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
            ...
            if max_val > 0.95:
                result = cv2.matchTemplate(
                    cv2.cvtColor(frame[75:110, 170:800], cv2.COLOR_BGR2GRAY),
                    template[75:110, 170:800],
                    cv2.TM_CCOEFF_NORMED,
                )
                min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(result)
                if max_val > 0.95:
