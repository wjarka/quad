...
class PlayerRenamer:
    def rename(self, current_name, name):
        ...
        existing_vocabulary = db.session.scalar(
            select(OcrVocabulary).where(OcrVocabulary.text_read == current_name)
        )
        if existing_vocabulary is None:
            ...
            db.session.commit()
        for game_model in db.session.scalars(
            select(GameModel).where(GameModel.player_name == current_name)
        ):
            self.rename_game(Game(model=game_model), name, "player_name")
        for game_model in db.session.scalars(
            select(GameModel).where(GameModel.opponent_name == current_name)
        ):
        ...
        if game.get("discord_message_id") is not None:
            ...
            try:
                webhook = discord.SyncWebhook.from_url(
                    current_app.config["DISCORD_WEBHOOK_GAMES"]
                )
                if message_id:
                    webhook.edit_message(
                        message_id=message_id, content=game.get_game_identifier()
                    )
...
class GamePathGenerator:
    def get_recording_path(self, game):
        return current_app.config["PATH_RECORDING"].format(
            **self._path_dictionary(game)
        )

    def get_screenshot_path(self, game):
        return current_app.config["PATH_SCREENSHOT"].format(
            **self._path_dictionary(game)
        )
